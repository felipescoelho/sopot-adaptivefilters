...
%   This script implements the SOPOT approximation in the NLMS adaptive
%   filter.
% Paper:
%
% Authors:
...
normH = norm(H);
gamma = 1e-4;  % small positive constant to initialize S_d
mu_sopot = [.13 .13 .13 .13 .13 .13 .13 .13];  % step size (SOPOT)
mu_fi = .13;  % step size (fixed-point)
...
S = struct('filterOrderNo', N-1, 'initCoeffs', w_fi(:, 1, 1, 1, 1),...
    'wordLength', word_len, 'maxPot', max_pot, 'gamma', gamma);
...
for mc = 1:ensemble
    ...
    for snr = 1:snr_len
        ...
        for spt = 1:max_sums
            ...
            [~, e_fi, w_fi(:, :, spt, snr, mc)] = NLMS_fixed_point(d, x.', S);
            
            % SOPOT NLMS
            S.step = mu_sopot(spt);
            S.maxSums = spt;
            S.maxSumsFilt = spt;
            [~, e_sopot, w_sopot(:, :, spt, snr, mc)] = NLMS_spt(d, x, S);
