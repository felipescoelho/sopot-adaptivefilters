...
%   This script implements the SOPOT approximation in the RLS adaptive
%   filter.
% Paper:
% Authors:
...
delta = .06;  % small positive constant to initialize S_d
lambda = [.975 .975 .96 .96 .955 .954 .96 .954];  % forgetting factor (SOPOT)
lambda_fi = .9987;  % forgetting factor (fixed-point)
...
S = struct('filterOrderNo', N-1, 'initCoeffs', w_fi(:, 1, 1, 1, 1),...
    'delta', delta, 'wordLength', word_len, 'maxPot', max_pot);
...
for mc = 1:ensemble
    ...
    for snr = 1:snr_len
        ...
        for spt = 1:max_sums
            ...
            [~, e_fi, w_fi(:, :, spt, snr, mc)] = RLS_Alt_fixed_point_2(d, x.', S);
            
            % SOPOT RLS
            S.lambda = lambda(spt);
            S.maxSums = spt;
            S.maxSumsFilt = spt;
            [~, e_sopot, w_sopot(:, :, spt, snr, mc)] = RLS_spt(d, x, S);
