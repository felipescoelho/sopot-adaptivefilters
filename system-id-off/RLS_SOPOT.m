function    [outputVector,...
             ...
             errorVectorPost,...
             km_w,...
             km_p_d] =   RLS_SOPOT(desired,input,S)
...
%           - adders                : Number of adders for the spt
%                                     approximation.
%
%   Output Arguments:
...
outputVectorPost        =   zeros(nIterations, 1);
errorVectorPost         =   zeros(nIterations, 1);


%   Initial State
S_d                     =   S.delta*eye(nCoefficients);
p_d                     =   zeros(nCoefficients, 1);
h_pd                    =   zeros(1, nCoefficients);
m_Sd                    =   S_d;
...
for it = 1:nIterations
    ...
    outputVector(it, 1) =   sopotVector(:, it)'*regressor;

    %   a priori error
    errorVector(it, 1)  =   desired(it)-outputVector(it, 1);

    S_d             =  (S.lambda)\((m_Sd)-((m_Sd)*regressor*(regressor')*(m_Sd))/...
                       (S.lambda+regressor'*(m_Sd)*regressor));
                   
                   
    [h_Sd, ~]       =  mpgbp_filter_approxP(S_d(:), nAdders, P);
    
    
    m_Sd = reshape(h_Sd, [nCoefficients, nCoefficients]);
    

    p_d             =  S.lambda*(h_pd')+conj(desired(it))*regressor;
    
    
    [h_pd, km_p_d]  =  mpgbp_filter_approxP(p_d, nAdders, P);
    

    coefficientVector(:, it+1)  =   S_d*(h_pd');
    
     %   SOPOT Cycle:
    [h_spt, km_w]                 =   mpgbp_filter_approxP(coefficientVector(:, it+1), nAdders, P);
    
    sopotVector(:, it+1)        =   (h_spt)';

    %    A posteriori estimated output
    outputVectorPost(it, 1)     =   sopotVector(:, it+1)'*regressor;
